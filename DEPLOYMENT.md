...
All routes accept **both** GET (Vercel Cron) and POST (Supabase pg_cron /
manual testing) with the same auth check, so either scheduler works:
- Vercel Cron sends `Authorization: Bearer $CRON_SECRET` automatically via GET.
- Supabase pg_cron/pg_net sends an `x-cron-secret` header via POST.

| Job | IST | UTC cron | Endpoint |
|-----|-----|----------|----------|
| Email queue drain | every 2 min | `*/2 * * * *` | `/api/email/process` |
| Lock windows | 23:59 | `29 18 * * *` | `/api/cron/lock` |
| Daily snapshot | 17:00 | `30 11 * * *` | `/api/cron/daily-report` |
| AI review | Fri 17:15 | `45 11 * * 5` | `/api/cron/ai-review` |
| Weekly summary | Fri 17:30 | `0 12 * * 5` | `/api/cron/weekly-report` |
...
**Supabase pg_cron alternative** (reliable header-based auth, and required on
Vercel Hobby since `*/2 * * * *` exceeds the Hobby once-per-day limit):
```sql
create extension if not exists pg_cron;
create extension if not exists pg_net;
select cron.schedule('qc-daily','30 11 * * *', $$
  select net.http_post(
    url := 'https://YOUR_APP/api/cron/daily-report',
    headers := jsonb_build_object('x-cron-secret','YOUR_CRON_SECRET')
  );$$);
-- repeat for weekly-report (0 12 * * 5), ai-review (45 11 * * 5), lock (29 18 * * *), email/process (*/2 * * * *)
```
The ready-to-use version of this lives in `0004_pg_cron_schedule.sql` —
uncomment it, substitute your URL and secret, and apply it.
